"""Unit tests for vision CLI event-dataset path."""
...
@pytest.mark.unit
def test_vision_cli_runs_short_nmnist_loop_with_dataset_factory(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    def _fake_build_loaders(_self: Any, cfg: Any) -> DatasetLoaders:
        frames = torch.rand(24, 4, 2, 8, 8, generator=torch.Generator().manual_seed(19))
    ...
    dataset_meta_path = run_dir / "dataset" / "dataset_meta.json"
    ...
    assert dataset_meta["name"] == "nmnist"
    assert dataset_meta["sensor_size"] == [8, 8, 2]
