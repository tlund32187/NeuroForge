...
__all__ = ["CudaMetricsMonitor", "ResourceMonitor"]
...
class CudaMetricsMonitor:
    """Stamp torch CUDA allocator metrics onto scalar-like events."""
    ...
    def on_event(self, event: MonitorEvent) -> None:
        """Enrich scalar/trial events with CUDA memory statistics."""
        if not self.enabled:
            return
        if event.topic not in (EventTopic.SCALAR, EventTopic.TRAINING_TRIAL):
            return
