"""``BizHawkClient`` — a vision-only :class:`IGameClient` over the bridge protocol.

The client speaks the :mod:`~neuroforge.game.clients.protocol` wire format to a
BizHawk Lua side-car over an injected :class:`~neuroforge.game.clients.transport.ITransport`.
It reads only screen pixels and returns frame-derived observations — it never
touches emulator RAM, preserving the vision-only invariant.

Lifecycle::

    client = BizHawkClient(BizHawkClientConfig(...))
    obs = client.reset()                 # accept + HELLO/WELCOME handshake + first FRAME
    step = client.step(ControllerAction(right=True))   # ACTION -> next FRAME
    client.close()                       # CLOSE + release transport

Game-over is inferred from pixels by a downstream extractor (Phase 1), so the
client itself reports ``terminated=False`` and only sets ``truncated`` when the
configured per-episode step budget is reached.
"""
...
from neuroforge.contracts.game import GameClientStep, GameObservation, ScreenFrame
...
if TYPE_CHECKING:
    ...
    from neuroforge.game.clients.transport import ITransport
...
class BizHawkClientConfig:
    ...
    mismatched script fails fast and clearly. Defaults are NES native (256×240
    RGB).
