"""NeuroForge bridge wire protocol (codec only — no I/O).

A tiny, explicit, little-endian framing used by both the Python client and the
BizHawk Lua side-car. Keeping the codec pure (no sockets) makes every message
type unit-testable with a round-trip and lets the loopback test act as a
"fake Lua" using the very same encoders.
...
order: bit ``i`` set ⇔ ``NINTENDO_BUTTONS[i]`` pressed.
"""
...
# Action payload: button bitmask(B) + flags(B).
_ACTION_STRUCT: str = "<BB"

# Map controller button display names to ControllerAction kwargs.
...
# ── HELLO / WELCOME (JSON geometry handshake) ──────────────────────────
