...
Entrypoint registered in ``pyproject.toml``::
    ...
    [project.scripts]
    neuroforge = "neuroforge.interfaces.cli.main:main"

Also works as ``python -m neuroforge.interfaces.cli.main``.
"""
...
def _resolve_evolution_checkpoint(*, raw: str | None, run_dir: Path) -> str | None:
    """Resolve the evolution checkpoint path for `
euroforge run``."""
    if raw is None:
...
def _resolve_torch_device(device: str) -> str:
    if device != "auto":
        return device
    from neuroforge.kernel.torch_utils import default_device
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    from neuroforge.observability.monitors.stability_monitor import (
        StabilityConfig,
        StabilityMonitor,
    )
    ...
    from neuroforge.applications.tasks.registry import get_task_spec
...
def _cmd_bench(args: argparse.Namespace) -> int:
    ...
    from neuroforge.contracts.messaging import EventTopic, MonitorEvent
    from neuroforge.interfaces.cli.commands.bench import BenchRunConfig, run_bench
    from neuroforge.kernel.determinism.mode import DeterminismConfig, apply_determinism
    from neuroforge.messaging.bus import EventBus
    from neuroforge.observability.monitors.artifact_writer import ArtifactWriter
...
def _add_bench_parser(sub: Any) -> None:
    from neuroforge.interfaces.cli.commands.bench import (
