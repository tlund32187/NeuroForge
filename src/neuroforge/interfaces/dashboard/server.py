...
def _parse_resource_monitor_cfg(body: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(raw_monitoring, dict):
        return default
    raw_resource = cast("dict[str, Any]", raw_monitoring).get("resource")
    ...
    return {
        "enabled": _as_bool(resource.get("enabled"), default["enabled"]),
        "every_n_steps": _as_int(
            resource.get("every_n_steps"),
            default["every_n_steps"],
            minimum=1,
        ),
        "include_process": _as_bool(
            resource.get("include_process"),
            default["include_process"],
        ),
        "include_system": _as_bool(
            resource.get("include_system"),
            default["include_system"],
        ),
        "include_gpu": _as_bool(resource.get("include_gpu"), default["include_gpu"]),
        "gpu_index": _as_int(resource.get("gpu_index"), default["gpu_index"], minimum=0),
    }
...
def _make_json_safe(obj: Any) -> Any:
    ...
    if isinstance(obj, dict):
        ...
        return {k: _make_json_safe(v) for k, v in mapping.items()}
    if isinstance(obj, (list, tuple)):
        seq = cast("Sequence[Any]", obj)
        return [_make_json_safe(item) for item in seq]
    return obj
...
async def _handle_train(request: web.Request) -> web.Response:
    ...
    from neuroforge.simulation.runtime.run_context import create_run_dir
    ...
    def _run_vision_backbone() -> None:
        ...
        from neuroforge.interfaces.cli.commands.vision import (
            VisionRunnerConfig,
        ...
        if isinstance(v_gates, list):
            v_gates = tuple(cast("Sequence[Any]", v_gates))
...
def _load_confusion_matrix_from_npy(path: Path) -> list[list[int]] | None:
    ...
    if not isinstance(rows_raw, list):
        return None
    out: list[list[int]] = []
    for row in cast("Sequence[Any]", rows_raw):
        if not isinstance(row, list):
            return None
        out.append([int(v) for v in cast("Sequence[Any]", row)])
    return out
...
def _load_confusion_matrix_from_json(path: Path) -> list[list[int]] | None:
    ...
    if isinstance(payload, dict):
        ...
        if "matrix" in payload_obj:
            matrix_raw = payload_obj.get("matrix")
        elif "confusion_matrix" in payload_obj:
            matrix_raw = payload_obj.get("confusion_matrix")
    if not isinstance(matrix_raw, list):
        return None
    out: list[list[int]] = []
    for row in cast("Sequence[Any]", matrix_raw):
        if not isinstance(row, list):
            return None
        out.append([int(v) for v in cast("Sequence[Any]", row)])
    return out
    ...
    for d in sorted(_ARTIFACTS_DIR.iterdir(), reverse=True):
        ...
        if cfg_path.is_file():
            try:
                ...
                cfg: dict[str, Any] = (
                    cast("dict[str, Any]", cfg_raw) if isinstance(cfg_raw, dict) else {}
                )
...
async def _handle_run_vision_event_sample(request: web.Request) -> web.Response:
    ...
    image_files: dict[str, Any] = (
        dict(cast("dict[str, Any]", image_files_raw)) if isinstance(image_files_raw, dict) else {}
    )
...
async def _handle_run_vision_summary(request: web.Request) -> web.Response:
    ...
    if benchmark_summary is not None:
        ...
        if isinstance(raw_throughput, dict):
            tp = cast("dict[str, Any]", raw_throughput)
            throughput["steps_per_sec"] = _to_float(tp.get("steps_per_sec"))
            throughput["ms_per_step"] = _to_float(tp.get("ms_per_step"))
            throughput["wall_ms"] = _to_float(tp.get("wall_ms"))
        ...
        if isinstance(raw_final, dict):
            final_metrics = cast("dict[str, Any]", raw_final)
        if isinstance(raw_best, dict):
            best_metrics = cast("dict[str, Any]", raw_best)
        ...
        if isinstance(result, dict):
            ...
            throughput["steps"] = _to_int(result_obj.get("steps"))
            final_metrics = {
                "loss": _to_float(result_obj.get("final_loss")),
                "accuracy": _to_float(result_obj.get("final_accuracy")),
            }
            best_metrics = {}
