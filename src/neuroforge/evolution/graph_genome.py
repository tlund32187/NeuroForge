...
Unlike :class:`~neuroforge.evolution.genome.PolicyGenome` (a fixed vector of
...
(from :class:`~neuroforge.evolution.innovations.InnovationRegistry`) so crossover
...
:func:`neuroforge.game.policies.graph_network.build_graph_policy_network`.
"""
...
from neuroforge.evolution.genome import Gene, GeneDef
...
class GraphGenome:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> GraphGenome:
        ...
        return cls(
            ...
            connections=connections,
            hyperparams=hyperparams,
            parent_ids=tuple(str(pid) for pid in parents) if isinstance(parents, list) else (),
        )
...
class GraphReproduction:
    ...
    Mirrors the default :class:`~neuroforge.evolution.engine.SimpleReproduction`
    ...
    :class:`~neuroforge.evolution.engine.EvolutionEngine`.
    """
