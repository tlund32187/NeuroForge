...
from neuroforge.evolution._serde import cast_json_object
...
def load_best_genome_checkpoint(path: str | Path) -> BestGenomeCheckpoint:
    ...
    genome = decode_genome(cast_json_object(best.get("genome")))
    metrics = {
        str(key): float(value)
        for key, value in cast_json_object(best.get("metrics", {})).items()
