...
from neuroforge.evolution._serde import (
    cast_json_object,
    rng_state_from_json,
    rng_state_to_json,
)
from neuroforge.evolution.genome import PolicyGenome
from neuroforge.evolution.genome_codec import decode_genome
...
class EvolutionEngine:
    ...
    def load_state(self, path: str | Path) -> EvolutionState:
        ...
        population = [decode_genome(cast_json_object(item)) for item in raw_population]
        ...
        if isinstance(best_raw, dict):
            genome = decode_genome(cast_json_object(best_raw["genome"]))
            result = FitnessResult(
                fitness=float(best_raw["fitness"]),
                metrics={
                    str(key): float(value)
                    for key, value in cast_json_object(best_raw.get("metrics", {})).items()
                },
